...
“颜色检测”页复用 `ColorDetection.c` 的 HSL `0–240` 定义、红色/黄色默认阈值、ROI 和四连通组件几何过滤。原图和黑底检测结果并排显示，合格组件会以红色或黄色掩码与白色外框标出。主界面可调整红黄 H/S/L、ROI 顶部/左右范围和每色最大组件数；“完整阈值…”窗口还可调整每种颜色的面积上下限、最小宽高、填充率与纵横比上下限。默认值与 `g_color_detection_red_threshold`、`g_color_detection_yellow_threshold` 对齐，调整仅作用于上位机预览。

### 参数配置导入导出

实时图像面板顶部的“导出处理配置”会将逆透视和颜色检测的全部参数保存为一个 UTF-8 JSON 文件；“导入处理配置”会先验证版本、数值范围、ROI、单应矩阵可逆性，再一次性应用两个页面的配置。该文件可与嵌入式代码提交版本一起保存，用于复现同一组处理参数。
...
测试覆盖了 TCP/串口常见的任意长度分片、损坏/缺失 CRC 帧的丢弃与重同步、RGB565 转换、示波器暂停与历史回看、逆透视/颜色检测、参数配置导入导出模型和题述三种 Printf 格式。
