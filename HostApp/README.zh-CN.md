...
- “示波器”页可手动开启或关闭；开启后将通过 IMGT 校验的数值通道按时间绘制为不同颜色的曲线，并提供放大、缩小、自动缩放和清除操作。
- “实时图像”中的“逆透视”和“颜色检测”页均可显示实时原图与处理结果，并提供上位机侧的图形化参数调整；不会改写原始帧或 PNG 保存内容。
...
- **放大 / 缩小**：同时调整时间窗和当前幅值视图；
- **自动缩放**：取消手动幅值范围，按当前可见数据的范围留白显示；
- **清除**：删除当前示波器缓存，不影响分级日志；
- **启用开关**：关闭时不会继续采样，重新开启后只记录新的合格数值数据。

鼠标悬停在曲线采样点附近会显示该点相对当前时刻的时间和所有通道幅值；在图内滚动滚轮即可放大或缩小当前时间/幅值视图。

## 实时图像处理

### 逆透视

“逆透视”页按 `CameraIPM.h` 中的 `CAMERA_FX/FY/CX/CY`、径向畸变和 `CAMERA_IPM_H` 进行与嵌入式 `raw pixel → ground` 方向相反的重采样，得到俯视图。页面始终并排显示接收原图和处理结果；可通过滑条调整近端 X、远端 X、横向半宽 Y，并通过数值框调整输出宽高。

标定原始分辨率为 `160×128`。若接收图像的分辨率不同，上位机会按宽高比例换算映射坐标以便预览；若需要测量级精度，应重新标定并同步更新嵌入式与 `HostApp/image_processing.py` 中的参数。

### 颜色检测

“颜色检测”页复用 `ColorDetection.c` 的 HSL `0–240` 定义、红色/黄色默认阈值、下方 ROI 和四连通组件面积过滤。原图和黑底检测结果并排显示，合格组件会以红色或黄色掩码与白色外框标出。红黄的 H/S/L 上下限、ROI 顶部百分比与最小组件面积均能在界面调整；调整仅作用于上位机预览。
...
测试覆盖了 TCP/串口常见的任意长度分片、损坏/缺失 CRC 帧的丢弃与重同步、RGB565 转换、示波器数据模型、逆透视/颜色检测模型和题述三种 Printf 格式。
